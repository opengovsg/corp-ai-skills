...
Every skill in this repo, in one place. The `skills-intake` Action regenerates this table from disk after each merge, so it can't drift — but the owner column is preserved from whatever is here, so edit that by hand when a skill changes hands.
