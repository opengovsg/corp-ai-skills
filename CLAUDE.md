...
- `skills-intake.yml` pushes to `main`, which branch protection blocks for the default `GITHUB_TOKEN`. It authenticates instead with a fine-grained PAT, `INTAKE_PUSH_TOKEN`, scoped to this repo only with Contents: Read and write and nothing else — owned by `cs-keith`, expiring 2027-08-25. Rotate it before then, or every merge that touches `skills/` will land without its nudge installed and `index.md` will silently drift, the way it did the first time this went unnoticed.
